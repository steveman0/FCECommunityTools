...
namespace FortressCraft.Community.Utilities
{
    ...
    public class MachineInventory
    {
        ...
        public void WriteInventory(BinaryWriter writer)
        {
            ...
            {
                Debug.LogWarning("Trying to write Machine Inventory but inventory is null? Writing empty!");
                writer.Write(0);
                writer.Write(0);
                return;
            }

            List<ItemBase> items = this.Inventory; //Cache a local copy to avoid rare threading IndexOutofRange
            ...
            {
                ItemFile.SerialiseItem(items[index], writer);
            }
        }
